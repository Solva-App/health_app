...
const createAddress = async (req, res, next) => {
  ...
  try {
    ...
    if (addressCount === 0) {
      ...
      await Address.update({ isDefault: false }, { where: { userId, isDefault: true }, transaction: t })
    }

    const address = await Address.create({ ...req.body, userId, isDefault }, { transaction: t })
...
const makeDefaultAddress = async (req, res, next) => {
  ...
  try {
    ...
    await targetAddress.update({ isDefault: true }, { transaction: t })
